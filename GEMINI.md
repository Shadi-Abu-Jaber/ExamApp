...
- **Technologies:**
  - **Backend:** Node.js, Express.js, PostgreSQL (`pg` pool in `backend/src/db.js` + repositories in `backend/src/repositories/`).
  - **Frontend:** React 19 (Vite), Bootstrap 5, OOP service layer (Config / Logger / Storage / Notify / AuthService / ApiGateway).
  - **Auth:** JWT issued on login/register (bcrypt-hashed passwords). The server enforces role + ownership on every mutating route; the client stores the token and sends it as a Bearer header. In mock mode there is no token (role gating is client-side only).

## Directory Structure

- `backend/`: Express server — routes for `/auth`, `/exams`, `/submissions`, `/users`. Backed by PostgreSQL; the DB is seeded from `backend/db/` (schema + seed run automatically by the Docker image).
...
npm run db:up         # start local PostgreSQL in Docker (seeds itself on first run)
npm run dev           # nodemon on src/server.js, default PORT=4000
```

The server verifies the DB connection on boot and exits with a clear message if PostgreSQL is unreachable.
...
### Backend
- **Routes:** `backend/src/routes/` — `exams.js`, `submissions.js`, `auth.js`, `users.js`.
- **DB:** `backend/src/db.js` — `pg` connection pool; data access in `backend/src/repositories/`.
- **Auth:** `backend/src/auth/` (JWT + bcrypt) and `backend/src/middlewares/auth.js` (`authRequired`, `requireRole`).
